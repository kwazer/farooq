...
if($ledger_type == 2  or $ledger_type == 6 or $ledger_type == 8)
{
...
if($ledger_type == 2)
	...
	$queryLedgerDetail = mysqli_query($con,"insert into ledger_details(ledger_id,father_name,address,guarantor,phone_no,ledger_tenure,phone_sec) select ledger_id,'$father_name','$address','$guarantor','$phone_no','$payment_cycle','$phonesec' from ledgers order by ledger_id desc limit 1");
...
if($ledger_type == 1 or $ledger_type == 7 or $ledger_type == 3)
{
	$query = mysqli_query($con,"insert into ledgers(ledger_name,ledger_type,opening_balance,ledger_date) values('$ledger_name',$ledger_type,$opening_bal,CURDATE())");
	if($query)
	{
