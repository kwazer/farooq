...
<!--					<tbody>-->
				...
				{
//					if($date_flag == 1)
				$query_show_daybook = mysqli_query($con,"select *,date(lastupdated) as lastdate,DATE_FORMAT(date,'%d-%m-%Y') as date from daybook left join sale_ledgers using (ledger_id)join transaction_type on transaction_type.type_id = daybook.transaction_type where date between '$date_search' and '$enddate' and transaction_type = 2 order by daybook.date desc ");
//else
//				$query_show_daybook = mysqli_query($con,"select *,date(update_time) as lastdate,DATE_FORMAT(date,'%d-%m-%Y') as date from daybook left join ledgers using (ledger_id)join transaction_type on transaction_type.type_id = daybook.transaction_type left join updatelog using (sno) where log_date = str_to_date('$date_search','%Y-%m-%d') or date(lastupdated) = str_to_date('$date_search','%Y-%m-%d') ");
				...
				{
//					if($date_flag == 1)
				$query_show_daybook = mysqli_query($con,"select *,date(lastupdated) as lastdate,DATE_FORMAT(date,'%d-%m-%Y') as date from daybook left join sale_ledgers using (ledger_id) join transaction_type on transaction_type.type_id = daybook.transaction_type where date = 'CURDATE()' and transaction_type = 2 order by daybook.date desc");
//				else
