...
echo "<big style = \"font-size:28px;color:grey;letter-spacing:2px;\" >".$ledger_type."</big><span style = \"cursor:pointer;color:brown;margin-left:10px;border:0px solid black;padding:3px;\" onclick = \"location.href='admin_page.php';\">Back</span><form action='bill_statements.php' method='GET' style = 'float:right;'><span style = 'font-size:14px;color:brown;padding:8px;'>Enter Date</span><input  type = 'date' placeholder = 'enter date' name = 'date'/><input type = 'submit'/></form><br><hr>";
?>
<?php
if(isset($date_search))
$query = mysqli_query($con,"select ledger_name,sum(amount),concat(ledger_tenure,ledger_id) as account_no,month(date),Year(date) from daybook join ledgers using(ledger_id) join ledger_details using (ledger_id) where NOT transaction_type between 9 and 14 and transaction_type <> 5 and month(date) = month(str_to_date('$date_search','%Y-%m-%d')) and year(date) = year(str_to_date('$date_search','%Y-%m-%d')) group by ledger_id,month(date),Year(date) ");
//$query = mysqli_query($con,"select ledger_name,sum(amount),concat(ledger_tenure,ledger_id) as account_no,month(date),Year(date) from daybook join ledgers using(ledger_id) join ledger_details using (ledger_id) where transaction_type <> 5 and month(date) = month(str_to_date('$date_search','%Y-%m-%d')) and year(date) = year(str_to_date('$date_search','%Y-%m-%d')) group by ledger_id,month(date),Year(date) ");
else
$query = mysqli_query($con,"select ledger_name,sum(amount),concat(ledger_tenure,ledger_id) as account_no,month(date),Year(date) from daybook join ledgers using(ledger_id) join ledger_details using (ledger_id) where NOT transaction_type between 9 and 14 and transaction_type <> 5 and month(date) = month(CURDATE()) and year(date) = year(CURDATE()) group by ledger_id,month(date),Year(date) ");
//$query = mysqli_query($con,"select ledger_name,sum(amount),concat(ledger_tenure,ledger_id) as account_no,month(date),Year(date) from daybook join ledgers using(ledger_id) join ledger_details using (ledger_id) where transaction_type <> 5 and month(date) = month(CURDATE()) and year(date) = year(CURDATE()) group by ledger_id,month(date),Year(date) ");
