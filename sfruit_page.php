...
echo "<span style = 'float:right;font-size:12px;'>Account No : <u>".$account."</u></span><h3 style = 'text-align:left;text-transform:capitalize;font-size:16px;color:brown;font-weight:normal;margin-top:0px;'>S/o $father_name<br>$address<br>Phone : $phone_no / $phone_2</h3>";
...
//}
$query = mysqli_query($con,"select sum(amount),DATE_FORMAT(watak_detail.date,'%d-%m-%Y') as date,sum(peti),sum(dabba),year(daybook.date) as year from daybook join watak_detail on watak_detail.watak_no = daybook.transaction_id where daybook.ledger_id = '$ledger_id' and transaction_type = 9 group by year(daybook.date) order by daybook.date desc");
...
$query_pay = mysqli_query($con,"select amount,DATE_FORMAT(date,'%d-%m-%Y') as date,transaction_type,transaction_id,narration from daybook where ledger_id = $ledger_id and transaction_type = 14");
while($r_pay = mysqli_fetch_array($query_pay))
{
//	$net_amount = $net_amount - $r_pay['amount'];
//	$tot_debt = $tot_debt + $r_pay['amount'];
//echo "<tr><td style = 'text-align:left;'>".$r_pay['date']."</td><td style = 'text-align:left;'>Payment</td><td style = 'font-size:13px;text-align:left;'>".$r_pay['narration']."</td><td colspan = '2'></td><td style = 'text-align:right;'>".$r_pay['amount']."</td><td></td><td style = 'text-align:right;'>".sprintf("%.2f",$net_amount)."</td></tr>";
	...
	"part" => $r_pay['narration'],
	"credit" => $r_pay['amount'],
"tran2" => "Receipt #".$r_pay["transaction_id"],
	);
}

//$query_pay = mysqli_query($con,"select amount,DATE_FORMAT(date,'%d-%m-%Y') as date,year(date) as year,sum(amount) as amount,transaction_type,transaction_id,narration from daybook where ledger_id = $ledger_id and transaction_type = 16 group by year(date)");
$query_pay = mysqli_query($con,"select DATE_FORMAT(date,'%d-%m-%Y') as date,year(date) as year,sum(amount) as amount from  goods_receipt_detail where ledger_id = $ledger_id group by year(date);");
while($r_pay = mysqli_fetch_array($query_pay))
{
//	$net_amount = $net_amount - $r_pay['amount'];
//	$tot_debt = $tot_debt + $r_pay['amount'];
...
for($x=0;$x<=$new_len;$x++)
{
	...
	{$net_amount = $net_amount + ($array_complete[$x][credit]*-1);
		...
		$array_complete[$x][credit] = $array_complete[$x][credit]*-1;
	}
	else
	{
...
//}
	...
	if($array_complete[$x][tran] == 'Baardana' or $array_complete[$x][tran] == 'Watak' or $array_complete[$x][tran] == 'Mandi Watak' or $array_complete[$x][tran] == 'goods_receipt')
	{
		...
		if($array_complete[$x][tran] === 'goods_receipt')
		{
//		echo "<tr><td colspan = '8' onclick = 'alert(\"its here\")'>clikc</td></tr>";
			echo "<tr  onclick = 'fill_gr(\"".$array_complete[$x][year]."\",16,\"".$array_complete[$x][ledger]."\")'><td style = 'text-align:left;'>".$array_complete[$x][date]."</td><td style = 'text-align:left;'>".$array_complete[$x][tran]."</td><td style = 'text-align:left;font-size:13px;'>".$array_complete[$x][part]."</td><td style = 'text-align:right;'>".$array_complete[$x][P]."</td><td style = 'text-align:right;'>".$array_complete[$x][H]."</td><td style = 'text-align:right;'>".$array_complete[$x][debt]."</td><td style = 'text-align:right;'>".$array_complete[$x][credit]."</td><td style = 'text-align:right;'>".sprintf("%.2f",$net_amount)."</td></tr>";
		}
	}
	else
	{
		if($array_complete[$x][tran] == 'receipt')
			echo "<tr ><td style = 'text-align:left;'>".$array_complete[$x][date]."</td><td style = 'text-align:left;'>".$array_complete[$x][tran2]."</td><td style = 'text-align:left;font-size:13px;'>".$array_complete[$x][part]."</td><td style = 'text-align:right;'>".$array_complete[$x][P]."</td><td style = 'text-align:right;'>".$array_complete[$x][H]."</td><td style = 'text-align:right;'>".sprintf("%.2f",$array_complete[$x][debt])."</td><td style = 'text-align:right;'>".sprintf("%.2f",$array_complete[$x][credit])."</td><td style = 'text-align:right;'>".sprintf("%.2f",$net_amount)."</td></tr>";
		else
			echo "<tr ><td style = 'text-align:left;'>".$array_complete[$x][date]."</td><td style = 'text-align:left;text-transform:capitalize;'>".$array_complete[$x][tran]."</td><td style = 'text-align:left;font-size:13px;'>".$array_complete[$x][part]."</td><td style = 'text-align:right;'>".$array_complete[$x][P]."</td><td style = 'text-align:right;'>".$array_complete[$x][H]."</td><td style = 'text-align:right;'>".$array_complete[$x][debt]."</td><td style = 'text-align:right;'>".sprintf("%.2f",$array_complete[$x][credit])."</td><td style = 'text-align:right;'>".sprintf("%.2f",$net_amount)."</td></tr>";
	}
}
