...
$query_se = mysqli_query($con,"select * From daybook join ledgers on ledgers.ledger_id = daybook.ledger_id where transaction_type = '$sno' order by transaction_id ");
echo "<table style = 'width:100%;' id = 'view_table'>";
echo "<th>date</th><th>serial #</th><th>Party Name</th><th>Bill No</th><th>Amount</th>";
while($row_se = mysqli_fetch_array($query_se))
{
	echo "<tr onclick = 'insert_details(this.rowIndex,".$row_se['transaction_id'].");'><td>".$row_se['date']."</td><td>".$row_se['transaction_id']."</td><td>".$row_se['ledger_name']."</td><td>".$row_se['narration']."</td><td>".$row_se['amount']."</td></tr>";
}
