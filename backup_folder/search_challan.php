...
{
//	echo $challan_id;
$query = mysqli_query($con,"select *,DATE_FORMAT(date,'%d-%m-%Y') as new_date from daybook join ledgers using (ledger_id) where transaction_type = 11 and narration = '$challan_id' order by date desc");
...
//	echo "<tr style = 'color:blue;' onclick = 'add_watak(".$row['transaction_id'].")'><td>".$row['new_date']."</td><td>".$row['narration']."</td><td>".$row['ledger_name']." <span style = 'color:brown;'> -B ".$row['ledger_id']."</span></td><td>".$row['transaction_id']."</td><td><button>Submit Challan</button></td></tr>";
	echo "<tr style = 'color:blue;'><td>".$row['new_date']."</td><td>".$row['narration']."</td><td>".$row['ledger_name']." <span style = 'color:brown;'> -B ".$row['ledger_id']."</span></td><td>".$row['transaction_id']."</td><td><button>Save Changes</button></td></tr>";
}
echo "</table>";
echo "<table style = 'width:100%;border-collapse:collapse;' border = \"1\">";
