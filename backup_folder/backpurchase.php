...
$query = mysqli_query($con,"select *,qty*dealer_rate as total from bill_buffer join items on bill_buffer.uid = items.item_id join author on items.author = author.author_id");
if($transaction_type == 1)
$query = mysqli_query($con,"select *,qty*cost_price as total from bill_buffer join items on bill_buffer.uid = items.item_id join author on items.author = author.author_id");
else
...
echo "<th>Book Name</th> <th>Qty</th> <th>Price</th><th>Author</th><th style = 'text-align:right;'>Total</th>";
while($row = mysqli_fetch_array($query))
{
if($transaction_type == 3)
{
echo "<tr><td>".$row['name']."</td><td><input style = 'width:50px;text-align:center;' value = '".$row['qty']."' type = 'text'/></td><td>".$row['dealer_rate']."</td><td>".$row['author_name']."</td><td style = 'text-align:right;'>".$row['total']."</td></tr>";
}
if($transaction_type == 1)
{
echo "<tr><td>".$row['name']."</td><td><input style = 'width:50px;text-align:center;' onKeyUp = 'if(event.keyCode == 13)alter_qty(".$row['sno'].",this.value,".$row['cost_price'].");' value = '".$row['qty']."' type = 'text'/></td><td>".$row['cost_price']."</td><td>".$row['author_name']."</td><td style = 'text-align:right;' id = 'p".$row['sno']."'>".$row['total']."</td></tr>";
}
else
{
echo "<tr><td>".$row['name']."</td><td><input onKeyUp = 'if(event.keyCode == 13)alter_qty(".$row['sno'].",this.value,".$row['cost_price'].");' style = 'width:50px;text-align:center;' value = '".$row['qty']."' type = 'text'/></td><td>".$row['retail_rate']."</td><td>".$row['author_name']."</td><td style = 'text-align:right;' >".$row['total']."</td></tr>";
}
...
echo "<tr style = 'border-top-style:solid;border-color:grey;border-width:1px;'><td></td><td></td><td></td><td style = 'font-weight:bold;'>Grand Total</td><td style = 'text-align:right;font-weight:bold;' id = 'grand_total'>$grand_total</td></tr>";
