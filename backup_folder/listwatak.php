...
$tran_id;
$query = mysqli_query($con,"select *,daybook.ledger_id as major_id from daybook join watak_detail on watak_detail.watak_no = daybook.transaction_id where transaction_type = 9 order by daybook.date desc");
...
{
	echo "<tr style = 'color:blue;' onclick = 'add_watak_entry(".$row['transaction_id'].")'><td>".$row['date']."</td><td>".$row['narration']."</td><td style = 'text-align:left;'>".$row['marka']."</td><td>".$row['transaction_id']."</td><td>".$row['peti']."</td><td>".$row['dabba']."</td><td>".$row['gross']."</td><td><div class = 'tooltip'>".$row['expenses']."<span class = 'tooltiptext' id = 'tool".$row['sno']."'>";
