...
if($type_id == 10){
	$query_bill_id = mysqli_query($con,"select bill_id from baardana_detail where sno = $sno");
	while($row = mysqli_fetch_array($query_bill_id))
	{
		$bill_id = $row['bill_id'];
	}
//$query = mysqli_query($con,"update sale_detail set uid = $uid,price = $price,qty = $qty,amount = price*qty where sno = $sno");
$query = mysqli_query($con,"delete from baardana_detail where sno = $sno");
$query_update = mysqli_query($con,"update daybook set amount = (select sum(amount) from baardana_detail where bill_id = $bill_id) where transaction_type = 10 and transaction_id = $bill_id");
}

if($type_id == 2){
	$query_bill_id = mysqli_query($con,"select bill_id from bill_detail where sno = $sno");
	while($row = mysqli_fetch_array($query_bill_id))
	{
		$bill_id = $row['bill_id'];
	}
//$query = mysqli_query($con,"update sale_detail set uid = $uid,price = $price,qty = $qty,amount = price*qty where sno = $sno");
$query = mysqli_query($con,"delete from bill_detail where sno = $sno");
$query_update = mysqli_query($con,"update daybook set amount = (select sum(amount) from bill_detail where bill_id = $bill_id) where transaction_type = 2 and transaction_id = $bill_id");
}
if($type_id == 1){
