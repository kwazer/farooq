...
//echo $other_ledger;
$arrayLedger = array();
echo "<form method = 'POST' action = 'backeditloan.php' onsubmit = \"return submit_changes(this)\">";
...
while($ro_ledger = mysqli_fetch_array($query_list_ledger))
{
 ...
 $query = mysqli_query($con,"select * from daybook join transaction_type on daybook.transaction_type = transaction_type.type_id join ledgers using (ledger_id) where transaction_type = 6 and transaction_id = $other_ledger");
...
echo "<input type = 'text' list = 'eexampleList' value = '".$row['ledger_name']."' name = 'eexample' id = 'eexample_input' oninput = 'showdatalisst_value()' style = 'width:100%;height:28px;' autofocus/>";
