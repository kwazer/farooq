...
//alert(cate);
		...
		xmlhttp.open("GET","backpublisheradd.php?pb_name="+pb_name,true);
		xmlhttp.send();
		}
		function addauthor()
		{
	...
	function show_itempage(publisher_name,publisher_id,item_id)
	{
if(flag == 1)
{
				var xmlhttp = new XMLHttpRequest();
		...
		xmlhttp.open("GET","edititem.php?item_id="+item_id+"&cat_name="+publisher_name+"&cat_id="+publisher_id,true);
		xmlhttp.send();
}
	}
...
function showpublishers()
{
	...
	xhr.open("GET","showpublishers.php",true);
	xhr.send();
	
}
function editpublisher(pb_id)
{
	var pb_name = document.getElementById("pb_name"+pb_id).value;
	//alert (pb_name);
		xhr = new XMLHttpRequest();
	xhr.onreadystatechange = function()
	{
...
//			document.getElementById("prose").innerHTML = xhr.responseText
	...
	xhr.open("GET","editpublishers.php?pb_id="+pb_id+"&pb_name="+pb_name,true);
	xhr.send();
}
	</script>
<?php 
...
$query = mysqli_query($con,"select * from publisher order by pb_name");
echo "<div style = \"\">";
echo "<table border = \"0\" style = \"border-collapse:collapse;\" cellspacing = \"2px\">";
echo "<tr><td class = 'lio' style = 'passing:1px;'><a href=\"itempage.php\" style = \"text-decoration:none;cursor:pointer;font-size:12px;display:block;background-color:white;padding:6px;text-transform:capitalize;border-radius:0px;border-bottom-style:solid;border-color:pink;border-width:1px;color:grey;\" >Edit Category</a></td></tr>";
while($row = mysqli_fetch_array($query))
{
	echo "<tr><td class = \"lio\" style = \"padding:1px;\"><a style = \"text-decoration:none;cursor:pointer;font-size:12px;display:block;background-color:white;padding:6px;text-transform:capitalize;border-radius:0px;border-bottom-style:solid;border-color:pink;border-width:1px;\" onclick = \"clio(".$row['pb_id'].",'".$row['pb_name']."')\">".$row['pb_name']."</a></td></tr>";
}
...
echo "<tr><td><input type = \"text\" id = \"cate\" placeholder = \"Add Category Here\" onkeypress = \"if(event.keyCode == 13)addpublisher();\"/></td></tr>";
...
<!--		<li style = "background:ivory;margin:5px;padding:10px;border-radius:10px 2px;"><span onclick = "showauthors()" style = "cursor:pointer;">Edit Authors</span></li>-->
		<li style = "background:ivory;margin:5px;padding:10px;border-radius:10px 2px;"><span onclick = "showpublishers()" style = "cursor:pointer;">Edit Category</span></li>
