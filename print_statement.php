...
while($ron = mysqli_fetch_array($query_opening))
{
	...
	$query_pre_tota = mysqli_query($con,"select IFNULL((select sum(amount) from daybook where ledger_id = $ledger_id and transaction_type = 9 and date < '$s_date'),0) + IFNULL((select sum(amount) from daybook where ledger_id = $ledger_id and transaction_type = 14 and date < '$s_date'),0) - IFNULL((select sum(amount) from daybook where ledger_id = $ledger_id and transaction_type = 13 and date < '$s_date'),0) + IFNULL((select sum(net) from challan_detail where ledger_id = $ledger_id and date < '$s_date'),0) - IFNULL((select sum(amount) from daybook where ledger_id = $ledger_id and transaction_type = 10 and date < '$s_date'),0) - opening_balance as balance from ledgers where ledger_type = 6 and ledger_id = $ledger_id");
...
echo "<tr style = 'text-align:center;' ><td colspan = '7' style = 'border:1px solid black;text-align:center;width:100%;color:black;'><span  style = 'float:right;color:black;font-weight:normal;font-size:12px;'><h4>T : 01951-254400<br>C: 919419434400<br>C: 919018184400<br>C: 919797274400</h4></span><h2 ><span id = 'font-face' style = 'letter-spacing:1px;'><a href= 'ledgers.php' style = 'text-decoration:none;'>FAROOQ FRUIT CO</a></span><br> <span style = 'font-size:15px;color:black;font-weight:normal;'><span style = 'color:black;padding:1px 4px;font-weight:bold;font-style:italic;border-radius:5px;'>Fruit Grower Commission & Forwarding Agents </span><br>55,56 Fruit & Vegetable Market<br>Zaloosa Charari-Sharief, Kashmir - 191112</span></h2></td></tr>";
echo "</table>";	
...
*/
$query = mysqli_query($con,"select sum(amount),DATE_FORMAT(daybook.date,'%d-%m-%Y') as date,sum(peti),sum(dabba),year(daybook.date) as year from daybook join watak_detail on watak_detail.watak_no = daybook.transaction_id where daybook.ledger_id = '$ledger_id' and transaction_type = 9 and daybook.date between '$s_date' and '$d_date' group by year(daybook.date) order by daybook.date desc");
...
$query_challan = mysqli_query($con,"select sum(net),sum(peti),sum(dabba),DATE_FORMAT(date,'%d-%m-%Y') as date,year(date) as year from challan_detail where challan_detail.ledger_id = $ledger_id and date between '$s_date' and '$d_date' group by year(date) order by date desc");
...
$query = mysqli_query($con,"select sum(amount),DATE_FORMAT(date,'%d-%m-%Y') as date,year(date) as year,transaction_type from daybook where ledger_id = '$ledger_id' and transaction_type = 10 and daybook.date between '$s_date' and '$d_date' group by year(date) order by date desc");
while($row = mysqli_fetch_array($query))
{
/*	$net_amount = $net_amount - $row['sum(amount)'];
...
$query_pay = mysqli_query($con,"select amount,DATE_FORMAT(date,'%d-%m-%Y') as date,transaction_type,narration from daybook where ledger_id = $ledger_id and transaction_type = 13 and daybook.date between '$s_date' and '$d_date' ");
...
$query_pay = mysqli_query($con,"select amount,DATE_FORMAT(date,'%d-%m-%Y') as date,transaction_type,narration from daybook where ledger_id = $ledger_id and transaction_type = 14 and daybook.date between '$s_date' and '$d_date'");
while($r_pay = mysqli_fetch_array($query_pay))
{
//	$net_amount = $net_amount - $r_pay['amount'];
//	$tot_debt = $tot_debt + $r_pay['amount'];
