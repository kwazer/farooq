...
{
	echo "<option value = '".$row_ledger['ledger_name']." S/o ".$row_ledger['father_name']." R/O ".$row_ledger['address']."' data-id = '".$row_ledger['ledger_id']."' id = '".$row_ledger['ledger_id']."'> F ".$row_ledger['ledger_id']."</option>";
...
echo "<span style = 'padding:2px 10px;margin-left:10px;border:1px solid grey;cursor:pointer;' onclick = 'location.href=\"printchallan.php?tran_id=$tran_id\"'>Print</span>";
...
{
	echo "<tr style = 'color:blue;' onclick = 'add_watak(".$row['transaction_id'].")'><td>".$row['new_date']."</td><td>".$row['narration']."</td><td style = 'text-align:left;'>".$row['ledger_name']." <span style = 'float:right;color:teal;'>B ".$row['ledger_id']."</span></td><td>".$row['transaction_id']."</td><td>".$row['truck_no']."</td></tr>";
}
echo "</table>";
