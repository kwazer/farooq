...
$query_opening = mysqli_query($con,"select *,sum(opening_balance),DATE_FORMAT(ledger_date,'%d-%m-%Y') as ledger_date from ledgers join ledger_details using (ledger_id)");
...
//	$tenure = $ron['ledger_tenure'];
	...
	//else
	$opening = $ron['sum(opening_balance)'];
...
$net_amount = 0;
echo '<div id = "main-window" style = "width:800px;border: 0px solid black;padding:10px;margin:0 auto;background:white;border-radius:10px;">';
...
//}
$query = mysqli_query($con,"select sum(amount),DATE_FORMAT(watak_detail.date,'%d-%m-%Y') as date,sum(peti),sum(dabba),year(daybook.date) as year from daybook join watak_detail on watak_detail.watak_no = daybook.transaction_id where transaction_type = 9 order by daybook.date desc");
...
$query_challan = mysqli_query($con,"select sum(net),sum(peti),sum(dabba),DATE_FORMAT(date,'%d-%m-%Y') as date,year(date) as year from challan_detail order by date desc");
...
$query = mysqli_query($con,"select sum(amount),DATE_FORMAT(date,'%d-%m-%Y') as date,year(date) as year,transaction_type,transaction_id from daybook where transaction_type = 10  order by date desc");
while($row = mysqli_fetch_array($query))
{
//	$net_amount = $net_amount - $row['sum(amount)'];
...
$query_pay = mysqli_query($con,"select sum(amount),DATE_FORMAT(date,'%d-%m-%Y') as date,transaction_type,narration,transaction_id from daybook where transaction_type = 13");
...
$query_pay = mysqli_query($con,"select sum(amount),DATE_FORMAT(date,'%d-%m-%Y') as date,transaction_type,transaction_id,narration from daybook where transaction_type = 14");
while($r_pay = mysqli_fetch_array($query_pay))
{
//	$net_amount = $net_amount - $r_pay['amount'];
//	$tot_debt = $tot_debt + $r_pay['amount'];
...
for($x=0;$x<=$new_len;$x++)
{
	...
	{
	if($array_complete[$x][tran] == 'Mandi Watak')
