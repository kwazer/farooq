...
$query = mysqli_query($con,"select sum(peti),sum(dabba),sum(expenses),sum(gross),sum(net_amount),watak_no,daybook.date from watak_detail join daybook on daybook.transaction_id = watak_id  where year(daybook.date) = '$year' and transaction_type = 9  group by watak_no,year(daybook.date)") ;
...
{
echo "<tr><td>".$row['sum(peti)']."</td><td>".$row['sum(dabba)']."</td><td>".$row['sum(gross)']."</td><td>".$row['sum(expenses)']."</td><td>".$row['net_amount']."</td></tr>";
/*
	$challan = $row[5];
	$array_challan2[] = $row;
*/
