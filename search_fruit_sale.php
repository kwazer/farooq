...
<?php
$query_ledger = mysqli_query($con,"select * from ledgers join ledger_details using (ledger_id) where ledger_type = 8");
...
echo "<tr><td><label style = 'font-size:18px;font-style:italic;'>Enter fruit Sale serial</label></td><td><input onkeypress = 'update_query(this.value)' style = 'height:28px;width:100%;' type = 'text' name = 'transaction_id' id = 'transaction_id'/></td><td><button value = 'Search' style = 'font-style:italic;' >Search</button></td></tr>";
echo "</table>";
if(isset($tran_id))
{
...
$query_search = mysqli_query($con,"select * from daybook join ledgers using (ledger_id) where transaction_type = 15 and transaction_id = '$tran_id'");
