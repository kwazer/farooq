...
while($ron = mysqli_fetch_array($query_opening))
{
	...
	while($pre_row = mysqli_fetch_array($query_pre_tota))
	{
		...
		if($opening_grade > 0)
		{
		$array_complete[0][debt] = $opening_grade;
...
/////////////////////////////////////////	
	...
	$query_pre_total = mysqli_query($con,"select IFNULL((select sum(amount) from daybook where ledger_id = $ledger_id and transaction_type = 3),0) + IFNULL((select sum(amount) from daybook where ledger_id = $ledger_id and transaction_type = 6),0) - IFNULL((select sum(amount) from receipt_detail where ledger_id = $ledger_id),0) - IFNULL((select sum(amount) from daybook where ledger_id = $ledger_id and transaction_type = 8),0) + opening_balance as balance from ledgers where ledger_type = 2 and ledger_id = $ledger_id");
...
echo "<h4 style = 'background:peachpuff;padding:5px;border-radius:20px;text-align:left;color:navy;font-size:17px;'>ELECTRONICS ACCOUNT <span style = 'margin-left:160px;font-size:22px;'><i><b>** Statement **</b></i></span><span style = 'float:right;'>Cr $pre_total</span></h4>";
}
else
{
	$pre_total = $pre_total * -1;
echo "<h4 style = 'background:peachpuff;padding:5px;border-radius:20px;text-align:left;color:navy;font-size:17px;'>ELECTRONICS ACCOUNT <span style = 'margin-left:160px;font-size:22px;'><i><b>** Statement **</b></i></span><span style = 'float:right;'>Dr $pre_total</span></h4>";
}
echo "<table style = 'width:100%;border-collapse:collapse;'>";
echo "<tr style = 'color:brown;border-width:1px;border-bottom-style:solid;border-color:peachpuff;'><td style = 'text-align:left;'>Date</td><td style = 'text-align:left;'>Transaction</td><td style = 'text-align:left;'># No</td><td style = 'text-align:left;'>Bill/Receipt No</td><td></td><td style = 'text-align:right;'>Debit</td><td style = 'text-align:right;'>Credit</td><td style = 'text-align:right;'>Balance</td></tr>";
$gr_peti =0 ;
$gr_dabba =0 ;
$tot_debt = 0;
$tot_cr = 0;
...
*/
$query = mysqli_query($con,"select *,DATE_FORMAT(date,'%d-%m-%Y') as date,sum(amount),year(date) as year,month(date) as month from receipt_detail where ledger_id = $ledger_id and date between '$s_date' and '$d_date' group by month(date),year(date)");
...
$query_challan = mysqli_query($con,"select *,DATE_FORMAT(date,'%d-%m-%Y') as date,year(date) as year from daybook where transaction_type = 6 and ledger_id = $ledger_id and date between '$s_date' and '$d_date'");
...
"tr_type" => 21,
"ledger" => $ledger_id,
"part" => "# ".$row['transaction_id'],
	"narration" => $row['narration'],
);

	
}
$query = mysqli_query($con,"select *,DATE_FORMAT(date,'%d-%m-%Y') as date,year(date) as year from daybook where transaction_type = 8 and ledger_id = $ledger_id and date between '$s_date' and '$d_date'");
while($row = mysqli_fetch_array($query))
{
/*	$net_amount = $net_amount - $row['sum(amount)'];
...
$query_pay = mysqli_query($con,"select *,DATE_FORMAT(date,'%d-%m-%Y') as date,year(date) as year from daybook where transaction_type = 3 and ledger_id = $ledger_id and date between '$s_date' and '$d_date'");
...
//}
	...
	echo "<tr ><td style = 'text-align:left;'>".$array_complete[$x][date]."</td><td style = 'text-align:left;'>".$array_complete[$x][tran]."</td><td style = 'text-align:left;font-size:13px;'>".$array_complete[$x][part]."</td><td style = 'text-align:right;'>".$array_complete[$x][P]."</td><td style = 'text-align:right;'>".$array_complete[$x][H]."</td><td style = 'text-align:right;'>".sprintf("%.2f",$array_complete[$x][debt])."</td><td style = 'text-align:right;'>".sprintf("%.2f",$array_complete[$x][credit])."</td><td style = 'text-align:right;'>".sprintf("%.2f",$net_amount)."</td></tr>";
			...
			*/
		echo "<tr ><td style = 'text-align:left;'>".$array_complete[$x][date]."</td><td style = 'text-align:left;'>".$array_complete[$x][tran]."</td><td style = 'text-align:left;font-size:13px;'>".$array_complete[$x][part]."</td><td style = 'text-align:left;font-size:13px;'>".$array_complete[$x][narration]."</td><td style = 'text-align:right;'>".$array_complete[$x][H]."</td><td style = 'text-align:right;'>".$array_complete[$x][debt]."</td><td style = 'text-align:right;'>".abs($array_complete[$x][credit])."</td><td style = 'text-align:right;'>".sprintf("%.2f",$net_amount)."</td></tr>";
