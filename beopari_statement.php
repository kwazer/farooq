...
{
$select_year = mysqli_query($con,"select distinct year(date) from challan_detail order by year(date) desc limit 1");
...
<h3 style = 'color:brown;'>>  Beopari Statement</h3>
<?php
...
$query = mysqli_query($con,"select sum(peti),sum(dabba),sum(expenses),sum(gross),sum(net),challan_id,daybook.date from challan_detail join daybook on daybook.transaction_id = challan_id  where year(daybook.date) = '$year' and transaction_type = 11  group by challan_id") ;
...
$sel_year = mysqli_query($con,"select distinct year(date) from challan_detail order by year(date) desc");
		...
		$array_challan[$challan][dabba] = $row['sum(dabba)'];
			$array_challan[$challan][expenses] = $row['sum(expenses)'];
				$array_challan[$challan][gross] = $row['sum(gross)'];
					$array_challan[$challan][net] = $row['sum(net)'];
}
...
$ledger_id = 0;
$query_daybook = mysqli_query($con,"select * from daybook join ledgers using (ledger_id) where transaction_type = 11 and year(date) = '$year'  order by ledger_id");
