...
$query_list_ledger = mysqli_query($con,"select * from ledgers where ledger_type = 3");
while($ro_ledger = mysqli_fetch_array($query_list_ledger))
{
 ...
 $query = mysqli_query($con,"select * from daybook join transaction_type on daybook.transaction_type = transaction_type.type_id join ledgers using (ledger_id) where transaction_type = 18 and transaction_id = $other_ledger");
