...
if($tr_type == 9)
{
	$query = mysqli_query($con,"select *,DATE_FORMAT(date,'%d-%m-%Y') as date from watak_detail where ledger_id = $led_id and year(date) = '$year'");
...
echo "<span style = 'float:right;'><button onclick = 'location.href = \"watakk_detail.php?tr_type=$tr_type&year=$year&led_id=$led_id\"' style = 'background-color:lightgreen;'>Print</button></span></p>";
echo "<table style = 'width:100%;background:white;'>";
echo "<tr style = 'padding:2px;'><th>Date</th><th>Watak No</th><th>Marka</th><th>Peti</th><th>Half</th><th style = 'text-align:right;'>Gross</th><th style = 'text-align:right;'>Expense</th><th style = 'text-align:right;'>Net</th></tr>";
while($row = mysqli_fetch_array($query))
{
	echo "<tr style = 'padding:2px;'><td>".$row['date']."</td><td onclick = ''>".$row['watak_no']."</td><td>".$row['marka']."</td><td>".$row['peti']."</td><td >".$row['dabba']."</td><td style = 'text-align:right;'>".$row['gross']."</td><td style = 'text-align:right;'>".$row['expenses']."</td><td style = 'text-align:right;'>".$row['net_amount']."</td></tr>";
